...
const UserPrograms = () => {
  const randomUsers = useQuery(api.usersRandom.getRandomUsers);
if (!randomUsers || randomUsers.length < 3) return null;
    ...
    <div className="w-full pb-24 pt-16 relative">
      ...
      {USER_PROGRAMS.map((program, index) => (
        ...
        >
          ...
          {/* Pass one random user to each card */}
          <ProgramShow user={user} randomUser={randomUsers[index]} />

          <CardContent className="px-5">
            {/* Your program details go here */}
          </CardContent>
        </Card>
      ))}
    </div>
